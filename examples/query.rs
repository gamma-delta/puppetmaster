...
struct MainState {
    x: f32,
    y: f32,
    controls: QueryInputHandler<KeyCode, Control>,
}

fn main() {
    let (ctx, event_loop) = ContextBuilder::new("query_example", "gamma-delta")
        .window_setup(WindowSetup {
            title: String::from("QueryInputHandler Example"),
            ..Default::default()
        })
        .build()
        .unwrap();

    let controls = QueryInputHandler::new_with_controls(vec![
